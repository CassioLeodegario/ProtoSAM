...
from dataloaders.PolypSuperDataset import PolypSuperDataset
...
def get_nii_dataset(_config):
    ...
    tr_parent = SuperpixelDataset(  # base dataset
        ...
        else _config["max_iters_per_load"],
...
def get_polyp_dataset(_config):
    tr_transforms = get_train_transforms(_config)
    return PolypSuperDataset(
        base_dir=_config["path"]["Polyp_Superpix"]["data_dir"],
        mode="train",
        image_size=_config["input_size"][0],
        transforms=tr_transforms,
        num_rep=2,
...
def get_dataset(_config):
    if _config["dataset"] == "Polyp_Superpix":
        return get_polyp_dataset(_config)
...
def main(_run, _config, _log):
    ...
    for sub_epoch in range(n_sub_epoches):
        ...
        for idx, sample_batched in enumerate(pbar):
            ...
            support_fg_mask = [
                [shot[f"fg_mask"].float().to(device, precision) for shot in way]
                for way in sample_batched["support_mask"]
            ]
            support_bg_mask = [
                [shot[f"bg_mask"].float().to(device, precision) for shot in way]
                for way in sample_batched["support_mask"]
            ]

            query_images = [
                query_image.to(device, precision)
            ...
            align_loss = (
                align_loss.detach().data.cpu().numpy() if align_loss != 0 else 0
            )
