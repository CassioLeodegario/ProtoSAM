...
def cfg():
    ...
    which_aug = (
        "sabs_aug"  # standard data augmentation with intensity and geometric transforms
    )
    ...
    label_sets = (
        0  # which group of labels taking as training (the rest are for testing)
    )
    curr_cls = ""  # choose between rk, lk, spleen and liver
    exclude_cls_list = [
        2,
        3,
    ]  # testing classes to be excluded in training. Set to [] if testing under setting 1
    ...
    clsname = None  #
    reload_model_path = (
        None  # path for reloading a trained model (overrides ms-coco initialization)
    )
    proto_grid_size = 8  # L_H, L_W = (32, 32) / 8 = (4, 4)  in training
    feature_hw = [
        input_size[0] // 8,
        input_size[0] // 8,
    ]  # feature map size, should couple this with backbone in future
    lora = 0
    use_3_slices = False
    do_cca = False
    ...
    ttt = False
    reset_after_slice = (
        True  # for TTT, if to reset the model after finetuning on each slice
    )
    model = {
        "align": usealign,
    ...
    task = {"n_ways": 1, "n_shots": 1, "n_queries": 1, "npart": n_sup_part}

    optim_type = "sgd"
    lr = 1e-3
    momentum = 0.9
    weight_decay = 0.0005
    optim = {"lr": lr, "momentum": momentum, "weight_decay": weight_decay}
    ...
    path = {
        ...
        "CHAOST2_Superpix_672": {
            "data_dir": "./data/CHAOST2/chaos_MR_T2_normalized_672/"
        },
        "SABS_Superpix_448": {"data_dir": "./data/SABS/sabs_CT_normalized_448"},
        "SABS_Superpix_672": {"data_dir": "./data/SABS/sabs_CT_normalized_672"},
        "Polyp_Superpix": {
            "data_dir": "/workspace/polyp_raw/TrainDataset/TrainDataset"
        },
    }
